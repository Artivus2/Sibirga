...
<template>
  <div class="order-route-map-container max-content" :class="maxWindowClass" @click.stop="activeModal = ''">
    ...
    <div class="main-content">
      ...
      <div class="table-header">
        <div class="table-column"
             @click.stop="sortOrders('id')">
          <span>Наименование участка подразделения</span>
        ...
        <div class="table-column"
          ...
          <span>Отправлен на согласование</span>
        ...
        <div class="table-column"
             @click.stop="sortOrders('accepted_worker_full_name')">
          <span>Утверждён РВН шахты</span>
          <div class="icon-sort-none" v-if="sortedField !== 'accepted_worker_full_name'"></div>
          <div class="icon-sort-down" v-else-if="filterProps.accepted_worker_full_name"></div>
          <div class="icon-sort-up" v-else></div>
        </div>
        <div class="table-column"
             @click.stop="sortOrders('report_done')">
          <span>Отчет сдан</span>
          <div v-if="sortedField !== 'report_done'" class="icon-sort-none"></div>
          <div v-else-if="filterProps.report_done" class="icon-sort-down"></div>
          <div v-else class="icon-sort-up"></div>
        </div>
        <div class="table-column"
             @click.stop="sortOrders('percent_complete')">
          <span>Выполнение</span>
          <div class="icon-sort-none" v-if="sortedField !== 'percent_complete'"></div>
          <div class="icon-sort-down" v-else-if="filterProps.percent_complete"></div>
          <div class="icon-sort-up" v-else></div>
        </div>
      </div>
      ...
      <div class="orders-container">
        ...
        <div class="table-body">
          <!-- Цикл по датам в которых хранятся наряды -->
          <div class="date-block" v-for="(allOrders, dateIdx) in ordersArray">
            <div class="order-date"
                 @click.stop="showDateTab(allOrders.date_time)">
              <span>{{ allOrders.date_time }}</span>
            </div>
            <!-- Цикл по нарядам в которых хранится смены -->
            <template v-if="activeOrders[allOrders.date_time]">
              <div class="table-row" v-for="order in allOrders.orders" :key="order.order_id"
                   @dblclick.stop="openModalOrderInfo(allOrders.date_time, order.order_id, order.shift_id)"
                   @contextmenu.prevent="showOrderContextMenu(allOrders.date_time_source, order, $event)"
              >

                <!-- Наименование участка/подразделения -->
                <div class="table-column"><span>{{ order.company_department_title }}</span></div>

                <!-- Смена -->
                <div class="table-column"><span>{{ order.shift_id }}</span></div>


                <!-- Создание наряда -->
                <template>
                  <div class="table-column"
                       v-if="order.last_state[50]">
                    <div class="green-column">
                                        <span>
                                            {{ order.last_status[50].full_name }}
                                        </span>
                      <span>
                                            {{ order.last_status[50].tabel_number }} | {{ order.last_status[50].position_title }}
                                        </span>
                    </div>
                  </div>
                  <!-- на случай отсутствия статуса создания наряда -->
                ...
                <template>
                  <!-- Наряд отправлен на согласование -->
                  <div class="table-column"
                       v-if="order.last_state[2]">
                    <div class="green-column">
                                        <span>
                                            {{ order.last_status[2].full_name }}
                                        </span>
                      <span>
                                            {{ order.last_status[2].tabel_number }} | {{ order.last_status[2].position_title }}
                                        </span>
                    </div>
                  </div>
                  <!--  Идет корректировка                -->
                  <div v-else-if="order.last_state[7]"
                       class="table-column">
                    <div class="orange-column">
                      <span> Идет корректировка... </span>
                    </div>
                  </div>
                  <!-- на случай отсутствия статуса создания наряда -->
                  <div class="table-column" v-else>
                ...
                <!-- Согласован АБ -->
                <div class="table-column"
                     v-if="order.last_state[4]">
                  <div class="green-column">
                    <span>{{ order.last_status[4].full_name }}</span>
                    <span>{{ order.last_status[4].tabel_number }} | {{ order.last_status[4].position_title }}</span>
                  </div>
                </div>
                <!-- Нет согласования АБ -->
                <div class="table-column"
                     v-else-if="!order.last_state[61]">
                  <div class="orange-column"
                       v-if="order.last_state[2]">
                    <span> Идет согласование... </span>
                  </div>
                </div>
                <!-- Чтобы отобразить треугольник с информацией об отклонении наряда, необходимо соблюсти одно из двух условий:
                     1) если есть статус 2(отправлен на согласование), и есть статус 61(наряд отклонен), и дата 61 статус позднее 2 статуса
                     2) если нет статуса 2 и есть статус 61
                 -->
                <div class="table-column"
                     v-else-if="order.last_state[61]">
                  <div class="red-triangle" :title="order.status_history[order.last_status[61].id].description">
                    <div class="red-triangle-exclamation-point">
                      !
                    </div>
                    <div class="red-triangle-text">
                                            <span>
                                                {{ order.last_status[61].full_name }}
                                            </span>
                      <span>
                                                {{ order.last_status[61].tabel_number }} | {{ order.last_status[61].position_title }}
                                            </span>
                    </div>
                  </div>
                </div>
                ...
                <div class="table-column">
                  <div class="alignment">
                    <span>{{ order.count_accept_worker }} чел.</span>
                  </div>
                </div>

                <!-- Утвержден РВН шахты -->
                <div class="table-column"
                     v-if="order.last_state[6]">
                  <div class="green-column">
                    <span>{{ order.last_status[6].full_name }}</span>
                    <span>{{ order.last_status[6].tabel_number }} | {{ order.last_status[6].position_title }}</span>
                  </div>
                </div>
                <!-- Нет согласования РВН -->
                <div class="table-column"
                     v-else-if="!order.last_state[10]">
                  <div v-if="order.last_state[2]"
                       class="orange-column">
                    <span> Идет утверждение... </span>
                  </div>
                </div>
                <!-- Чтобы отобразить треугольник с информацией об отклонении наряда, необходимо соблюсти одно из двух условий:
                     1) если есть статус 2(отправлен на согласование), и есть статус 10(наряд отклонен), и дата 10 статус позднее 2 статуса
                     2) если нет статуса 2 и есть статус 10
                 -->
                <div v-else-if="order.last_state[10]"
                     class="table-column">
                  <div :title="order.status_history[order.last_status[10].id].description" class="red-triangle">
                    <div class="red-triangle-exclamation-point">
                      !
                    </div>
                    <div class="red-triangle-text">
                      <span>
                        {{ order.last_status[10].full_name }}
                      </span>
                      <span>
                        {{ order.last_status[10].tabel_number }} | {{ order.last_status[10].position_title }}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <!-- Отчет сдан -->
                <div v-if="order.last_status && Object.keys(order.last_status).length &&
                                                     (order.last_status[8])"
                     class="table-column">
                  <div class="green-column">
                    <span>{{ order.last_status[8].full_name }}</span>
                    <span>{{ order.last_status[8].tabel_number }} | {{ order.last_status[8].position_title }}</span>
                  </div>
                </div>
                <div v-else class="table-column">
                  <div class="">
                    <span> </span>
                    <span> </span>
                  </div>
                ...
                <!-- Выполнение -->
                <div class="table-column" v-if="order.percent_complete">
                  <div class="indicator">
                    <div class="indicator-done"
                         :style="calcColorDone(order.percent_complete)"></div>
                    <div class="indicator-undone"
                         :style="calcWidthUnDone(order.percent_complete)"></div>
                  </div>
                  <div class="text">
                    <span :style="setTextColorIndicator(order.percent_complete)">{{ order.percent_complete }}%</span>
                  </div>
                </div>
                <div class="table-column percent"
                     v-else>
                  <div>
                    0%
                  </div>
...
import localStore from "../components/order-route-map-mine/orderRouteMapStore";
import OrderInformation from "../components/order-route-map-mine/orderInformation";
import OrderContextMenu from "@OS/components/order-route-map-mine/OrderContextMenu";
import MenuTileWithDropdown from "@/modules/order-vtb-mine/components/dropdowns/MenuTileWithDropdown.vue";
import orderVtbStore from "@/modules/order-vtb-mine/orderVtbStore";
...
const baseHeader = () => import('@/components/base/BaseHeader.vue'),
    ...
    calendar = () => import('@/modules/prof-security/components/injury-accounting/modals/CalendarMonthYearsModal.vue');

export default {

  name: "order-route-map",
  ...
  data() {
    return {
      maxWindowClass: '',                                                                                     // переменная для переключения полноэкранного режима
...
.order-route-map-container {
  ...
  border: 2px solid rgb(77, 137, 124);
  border-top: none;
...
.icon-sort-down {
  width: 20px;
  height: 20px;
  background: url(../src/assets/Arrow_down_grey.png) center no-repeat;
  background-size: 100% 100%;
}
...
.icon-sort-up {
  width: 20px;
  height: 20px;
  background: url(../src/assets/Arrow_up_grey.png) center no-repeat;
  background-size: 100% 100%;
}
