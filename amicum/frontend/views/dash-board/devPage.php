...
$files = scandir($_SERVER['DOCUMENT_ROOT'] . "/web/dash_board_dev");

foreach ($files as $file) {

    if (preg_match('/^[A-Za-z0-9]*\.js$/', $file)) {
        $this->registerJsFile('/dash_board_dev/' . $file . '?ver=' . $version, ['depends' => [AppAsset::className()], 'position' => View::POS_END]);
    }
}

if (is_dir($_SERVER['DOCUMENT_ROOT'] . "/web/dash_board_dev/js")) {
    $files = scandir($_SERVER['DOCUMENT_ROOT'] . "/web/dash_board_dev/js");

    foreach ($files as $file) {
        $this->registerJsFile('/dash_board_dev/js/' . $file . '?ver=' . $version, ['depends' => [AppAsset::className()], 'position' => View::POS_END]);
    }
}

if (is_dir($_SERVER['DOCUMENT_ROOT'] . "/web/dash_board_dev/css")) {
    $files = scandir($_SERVER['DOCUMENT_ROOT'] . "/web/dash_board_dev/css");

    foreach ($files as $file) {
        $this->registerCssFile('/dash_board_dev/css/' . $file . '?ver=' . $version, ['depends' => [AppAsset::className()], 'position' => View::POS_END]);
    }
}
?>
