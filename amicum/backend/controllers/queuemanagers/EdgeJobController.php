 ...
 * Класс/Испольнитель очереди выработок
 * @package backend\controllers\queuemanagers
 */
class EdgeJobController extends BaseObject implements \yii\queue\JobInterface
{
	public $controller;                                                                                                 // название контроллера
	public $method;                                                                                                     // название метода
	public $data = array();                                                                                                       // входые параметры
	...
	public function execute($queue)
	{
		...
		QueueLog::AddLog($this->controller, $this->method, $this->data, $duration, $result, $errors, 'edge');// записываем лог в БД
